...
def setup_logger(log_file):
    ...
    # Stream handler for stdout with UTF-8 encoding
    sh = logging.StreamHandler(sys.stdout)
    sh.setLevel(logging.DEBUG)
    sh.setFormatter(formatter)
    try:
        # Reopen sys.stdout with UTF-8 encoding if possible
        sh.stream = open(sys.stdout.fileno(), mode='w', encoding='utf-8', buffering=1)
    except Exception:
        # If this fails, it will fall back to default encoding
        pass
