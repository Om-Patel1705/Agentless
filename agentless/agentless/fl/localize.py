...
def localize(args):
    # Check if the dataset argument is one of the known Hub datasets or the specific windows path
    # Or if it's our custom file path
    known_datasets = [
        "princeton-nlp/SWE-bench_Lite",
        "princeton-nlp/SWE-bench_Verified",
        "C:\\Users\\omnen\\OneDrive\\Desktop\\Project_SE\\repo_structure\\repo_structures",
    ]  # The hardcoded path from choices

    if args.dataset in known_datasets:
        # If it's a known dataset, load using the Hugging Face library
        print(f"Loading standard dataset '{args.dataset}' from source...")
        swe_bench_data = load_dataset(args.dataset, split="test")
    elif os.path.exists(args.dataset):
        # If it's not a known dataset, but the file path exists, assume it's our custom JSONL
        print(f"Loading custom dataset instances from local file: {args.dataset}")
        try:
            # Use the utility function to load the list of dictionaries from the JSONL
            swe_bench_data = load_jsonl(args.dataset)
            if not swe_bench_data:
                # Handle case where file is empty or load_jsonl returns None/empty list
                raise ValueError(
                    f"Custom dataset file '{args.dataset}' is empty or could not be loaded correctly."
                )
            print(f"Loaded {len(swe_bench_data)} instance(s) from custom file.")
        except Exception as e:
            # Catch any errors during file loading
            print(f"Error loading custom dataset file '{args.dataset}': {e}")
            raise SystemExit(f"Failed to load custom dataset: {e}")  # Exit if custom load fails
    else:
        # If it's not a known dataset and the file doesn't exist, raise an error
        raise FileNotFoundError(
            f"Dataset path '{args.dataset}' not found or is not a recognized dataset."
        )
...
def main():
    ...
    parser.add_argument(
        ...
        choices=[
            "my_datasets/my_small_dataset.jsonl",
            "princeton-nlp/SWE-bench_Lite",
            "princeton-nlp/SWE-bench_Verified",
            "C:\\Users\\omnen\\OneDrive\\Desktop\\Project_SE\\repo_structure\\repo_structures",
        ],
