...
from agentless.util.postprocess_data import extract_code_blocks # ADD THIS
...
# ... (Keep GitHub Problem Description and Repository Structure sections) ...

Analyze the problem description and identify **only the essential files** (from the provided Repository Structure) that require modification to fix the specific bugs mentioned (calculation in utility, formatting currency).
Do not include files that are unrelated to these specific issues.

Important:
- Only include those files Which could be related to the problem statement ONLY ONLY ONLY.
- Only select file names that exactly match those listed in the Repository Structure.
- Do not generate or assume new paths or file names.
- Do not include folder paths, line numbers, or descriptions.
- Return only the exact file names (as listed) on separate lines.

Wrap the result with triple backticks, like this:
```
file1.py
file2.py
```
...
"""
    ...
    # Check if content is None or empty/whitespace string BEFORE splitting
        if not content or not content.strip():
            return []
    # If content is present, strip and split
    ...
    def localize_irrelevant(self, top_n=1, mock=False):
        ...
        if model_identified_files_folder is None:
            self.logger.error("Model returned None when parsing irrelevant files. Skipping...")
            return [], None, traj
    ...
    def localize(self, top_n=1, mock=False) -> tuple[list, list, list, any]:
        ...
        traj["prompt"] = message
        raw_output = traj["response"]
        # Extract code blocks (returns a list)
        extracted_content_list = extract_code_blocks(raw_output)

        # --- FIX: Get the string content FROM the list ---
        content_string_to_parse = "" # Default to empty string
        if isinstance(extracted_content_list, list) and extracted_content_list:
            # If it's a non-empty list, take the first element
            content_string_to_parse = extracted_content_list[0]
            self.logger.info(f"Extracted content string: '{content_string_to_parse}'")
        elif isinstance(extracted_content_list, str):
             # Handle case where it might sometimes return a string directly
             content_string_to_parse = extracted_content_list
             self.logger.info(f"Extracted content string (was already str): '{content_string_to_parse}'")
        else:
            self.logger.warning(f"extract_code_blocks returned unexpected or empty content: {extracted_content_list}")


        # Now parse the extracted string (or empty string if nothing was found)
        model_found_files = self._parse_model_return_lines(content_string_to_parse)
        files, classes, functions = get_full_file_paths_and_classes_and_functions(
            self.structure
        )
        self.logger.info(f"DEBUG: Files parsed from model response: {model_found_files} (Type: {type(model_found_files)})")

        # --- FIX: Get actual filenames directly from the structure keys ---
        # The keys of the main structure dict ARE the filenames
        actual_filenames = list(self.structure.keys())
        self.logger.info(f"DEBUG: Actual filenames retrieved from structure keys: {actual_filenames} (Type: {type(actual_filenames)})")

        # Call the validation function using the CORRECT list of actual filenames
        found_files = model_found_files
        # found_files = correct_file_paths(model_found_files,files)
        self.logger.info(f"DEBUG: Files after correct_file_paths validation: {found_files} (Type: {type(found_files)})")
        return (
            found_files,
            {"raw_output_files": raw_output},
            traj,
