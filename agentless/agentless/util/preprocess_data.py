...
def show_project_structure(structure, spacing=4):
    pp_string = ""
    indent = " " * spacing
    for key, value in structure.items():
        # <<< ADD THIS CHECK >>>
        # Skip items where the value is not a dictionary (like our "diff": None)
        if not isinstance(value, dict):
            continue # Go to the next key-value pair

        # --- Original Logic (Now safe because value is confirmed to be a dict) ---
        pp_string += indent + key + "\n"

        # Check if 'classes' key exists in the dictionary 'value'
        if "classes" not in value: # This line (395) is now safe
            # Add recursive call only if value is a dictionary
            pp_string += show_project_structure(value, spacing + 4) # Line 396
        else:
            # Original logic for when "classes" IS in value
            for c in value["classes"]:
                pp_string += (indent + " " * 4) + f"class {c}\n"
            for f in value["functions"]:
                pp_string += (indent + " " * 4) + f"function {f}\n"
    return pp_string
...
def filter_none_python(structure):
    # Use a list to collect keys to delete after iterating
    keys_to_delete = []
    for key, value in structure.items():

        # --- PRIMARY SAFETY CHECK ---
        # Check if the value is a dictionary BEFORE doing anything else with it
        if not isinstance(value, dict):
            # If value is None, not a dict, etc., it doesn't fit the expected structure.
            # Mark the key for deletion. This prevents the AttributeError.
            keys_to_delete.append(key)
            continue # Skip to the next item in the loop

        # --- Logic for Dictionary Values ---
        # Now we know 'value' is a dictionary. Decide if it represents a file or a directory/node.
        # Heuristic: If it contains a "text" key, assume it's meant to be a file entry.
        is_file_like = "text" in value

        if is_file_like:
            # If it looks like a file entry, check the filename (key)
            if not key.endswith(".py"):
                # If the key doesn't end with .py, mark it for deletion
                keys_to_delete.append(key)
            # We don't need to recurse further into the file's structure for this filter's purpose
        else:
            # If it's not file-like (e.g., a directory node), recurse into it
            filter_none_python(value)
            # After recursing, check if the sub-structure became empty
            if not value: # Check if the dictionary 'value' is now empty
                keys_to_delete.append(key)

    # --- Perform Deletions ---
    # Delete all marked keys after finishing the loop
    for key in keys_to_delete:
        # Double-check key exists before deleting (safe practice)
        if key in structure:
            del structure[key]
